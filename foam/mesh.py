...
@dataclass
class GaussianMixtureSignalGenerator(SignalGenerator):
    """Generate a scalar field from a mixture of isotropic Gaussian kernels."""

    num_components: int = 3
    mean_variance: float = 0.05
    variance_std: float = 0.01
    ...
    def _sample_centers(self, mesh: "Mesh", rng: np.random.Generator) -> np.ndarray:
        if self.centers is not None:
            centers = np.asarray(self.centers, dtype=np.float64)
            if centers.shape[1] != mesh.points.shape[1]:
                raise ValueError("Provided centers dimensionality does not match mesh points")
            if centers.shape[0] != self.num_components:
                raise ValueError("Number of centers must match num_components")
            return centers
        mins = mesh.points.min(axis=0)
        maxs = mesh.points.max(axis=0)
        return rng.uniform(mins, maxs, size=(self.num_components, mesh.points.shape[1]))

    def _sample_variances(self, rng: np.random.Generator) -> np.ndarray:
        variances = rng.normal(loc=self.mean_variance, scale=self.variance_std, size=self.num_components)
        variances = np.clip(variances, a_min=1e-6, a_max=None)
        return variances

    def generate(self, mesh: "Mesh") -> TensorData:
        rng = self._rng()
        centers = self._sample_centers(mesh, rng)
        variances = self._sample_variances(rng)
        diffs = mesh.points[:, None, :] - centers[None, :, :]
        squared_distances = np.sum(diffs ** 2, axis=2)
        # For an isotropic Gaussian: exp(-0.5 * d^2 / sigma^2)
        responses = np.exp(-0.5 * squared_distances / variances[None, :])
        tensor = torch.from_numpy(responses.sum(axis=1, keepdims=True).astype(np.float32))
        return TensorData(tensor, rank=0)
